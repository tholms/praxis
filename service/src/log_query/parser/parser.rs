...
use nom::branch::alt;
use nom::bytes::complete::{escaped, is_a, tag, tag_no_case, take_while1};
use nom::character::complete::{
    digit1, hex_digit1, i32, i64, multispace0, multispace1, none_of, one_of, u32, u64,
};
...
fn option_literal(i: &str) -> IResult<&str, OptionLiteral> {
    alt((
        ...
        map(take_while1(|c: char| !c.is_whitespace()), |s: &str| {
            OptionLiteral::String(s.to_string())
        }),
    ))
    .parse(i)
}
...
fn options(i: &str) -> IResult<&str, Options> {
    map(
        separated_list0(
            multispace1,
            separated_pair(identifier, trim(tag("=")), option_literal),
        ),
...
fn options_with_comma_and_quoted(i: &str) -> IResult<&str, Options> {
    map(
        separated_list0(
            tag(","),
            separated_pair(trim(identifier), tag("="), trim(option_quoted_literal)),
        ),
        |x| x.into_iter().collect(),
    )
    .parse(i)
...
fn type_mapping(i: &str) -> IResult<&str, Vec<(String, Type)>> {
    separated_list1(
        tag(","),
        separated_pair(trim(identifier), tag(":"), trim(type_tag)),
    )
    .parse(i)
}
...
fn wildcard_identifier(i: &str) -> IResult<&str, String> {
    map(take_while1(is_kql_wildcard_identifier), |i: &str| {
        i.to_string()
    })
    .parse(i)
}
    ...
    alt((
        map(
            recognize(tuple((
                ...
                digit1,
                opt(pair(tag("."), digit1)),
                opt(tuple((tag("e"), opt(tag("-")), digit1))),
            ))),
            |x: &str| Some(x.parse().unwrap()),
        ),
...
fn dynamic(i: &str) -> IResult<&str, Option<Dynamic>> {
    alt((
        map(
            delimited(tag("["), separated_list0(tag(","), trim(dynamic)), tag("]")),
            |x| Some(Dynamic::Array(x)),
        ),
        map(
            delimited(
                tag("{"),
                separated_list0(
                    tag(","),
                    separated_pair(trim(string), tag(":"), trim(dynamic)),
                ),
                tag("}"),
            ),
            |x| Some(Dynamic::Dictionary(x.into_iter().collect())),
        ),
        map(
            preceded(tag("bool"), delimited(tag("("), trim(boolean), tag(")"))),
            |x| Some(Dynamic::Bool(x)),
        ),
        map(
            preceded(tag("datetime"), delimited(tag("("), trim(date), tag(")"))),
            |x| Some(Dynamic::DateTime(x)),
        ),
        map(
            preceded(tag("decimal"), delimited(tag("("), trim(decimal), tag(")"))),
            |x| Some(Dynamic::Decimal(x)),
        ),
        map(
            preceded(tag("int"), delimited(tag("("), trim(integer), tag(")"))),
            |x| Some(Dynamic::Int(x)),
        ),
        map(
            preceded(tag("long"), delimited(tag("("), trim(long), tag(")"))),
            |x| Some(Dynamic::Long(x)),
        ),
        map(
            preceded(
                alt((tag("timespan"), tag("time"))),
                delimited(tag("("), trim(timespan), tag(")")),
            ),
            |x| Some(Dynamic::Timespan(x)),
        ),
        map(preceded(tag_no_case("0x"), hex_digit1), |x| {
            Some(Dynamic::Long(Some(i64::from_str_radix(x, 16).unwrap())))
        }),
        map(
            terminated(decimal_number, alt((tag("days"), tag("day"), tag("d")))),
            |x| {
                Some(Dynamic::Timespan(Some(dec_to_i64(
                    x,
        ...
        map(
            terminated(decimal_number, alt((tag("hours"), tag("hour"), tag("h")))),
            |x| {
                Some(Dynamic::Timespan(Some(dec_to_i64(
                    x,
        ...
        map(
            terminated(
                decimal_number,
                alt((tag("minutes"), tag("minute"), tag("m"))),
            ),
            |x| {
                Some(Dynamic::Timespan(Some(dec_to_i64(
                    x,
        ...
        map(
            terminated(
                decimal_number,
                alt((tag("seconds"), tag("second"), tag("s"))),
            ),
            |x| Some(Dynamic::Timespan(Some(dec_to_i64(x, 1000 * 1000 * 1000)))),
        ),
        map(
            terminated(
                decimal_number,
            ...
            |x| Some(Dynamic::Timespan(Some(dec_to_i64(x, 1000 * 1000)))),
        ),
        map(
            terminated(
                decimal_number,
                alt((tag("microseconds"), tag("microsecond"), tag("micro"))),
            ),
            |x| Some(Dynamic::Timespan(Some(dec_to_i64(x, 1000)))),
        ),
        map(
            terminated(decimal_number, alt((tag("ticks"), tag("tick")))),
            |x| Some(Dynamic::Timespan(Some(dec_to_i64(x, 100)))),
        ),
        map(
            recognize(tuple((
                opt(tag("-")),
            ...
            |x: &str| Some(Dynamic::Real(Some(x.parse().unwrap()))),
        ),
        map(
            recognize(tuple((
                opt(tag("-")),
...
fn integer(i: &str) -> IResult<&str, Option<i32>> {
    alt((
        map(preceded(tag_no_case("0x"), hex_digit1), |x| {
            Some(i32::from_str_radix(x, 16).unwrap())
        }),
        map(i32, |x| Some(x)),
        map(tag("null"), |_| None),
    ))
...
fn long(i: &str) -> IResult<&str, Option<i64>> {
    alt((
        map(preceded(tag_no_case("0x"), hex_digit1), |x| {
            Some(i64::from_str_radix(x, 16).unwrap())
        }),
        map(i64, |x| Some(x)),
        map(tag("null"), |_| None),
    ))
...
fn timespan(i: &str) -> IResult<&str, Option<i64>> {
    alt((
        map(
            terminated(
                decimal_number,
                pair(multispace0, alt((tag("days"), tag("day"), tag("d")))),
            ),
            |x| Some(dec_to_i64(x, 1000 * 1000 * 1000 * 60 * 60 * 24)),
        ),
        map(
            terminated(
                decimal_number,
                pair(multispace0, alt((tag("hours"), tag("hour"), tag("h")))),
            ),
            |x| Some(dec_to_i64(x, 1000 * 1000 * 1000 * 60 * 60)),
        ),
        map(
            terminated(
                decimal_number,
                pair(multispace0, alt((tag("minutes"), tag("minute"), tag("m")))),
            ),
            |x| Some(dec_to_i64(x, 1000 * 1000 * 1000 * 60)),
        ),
        map(
            terminated(
                decimal_number,
                pair(multispace0, alt((tag("seconds"), tag("second"), tag("s")))),
            ),
            |x| Some(dec_to_i64(x, 1000 * 1000 * 1000)),
        ),
        map(
            terminated(
                decimal_number,
        ...
        map(
            ...
            |x| Some(dec_to_i64(x, 1000)),
        ),
        map(
            terminated(
                decimal_number,
                pair(multispace0, alt((tag("ticks"), tag("tick")))),
            ),
            |x| Some(dec_to_i64(x, 100)),
        ),
        map(
            tuple((
                separated_pair(i64, tag("."), separated_pair(u64, tag(":"), u64)),
                opt(preceded(tag(":"), decimal_number)),
            )),
            |((d, (h, m)), s)| {
                Some(
                    ((d * 24 + h as i64) * 60 + m as i64) * (1000 * 1000 * 1000 * 60)
                        + s.map(|x| dec_to_i64(x, 1000 * 1000 * 1000)).unwrap_or(0) as i64,
                )
            },
        ),
        map(
            ...
            |((h, m), s)| {
                Some(
                    (h as i64 * 60 + m as i64) * (1000 * 1000 * 1000 * 60)
                        + s.map(|x| dec_to_i64(x, 1000 * 1000 * 1000)).unwrap_or(0) as i64,
                )
            },
        ),
...
fn literal(i: &str) -> IResult<&str, Literal> {
    alt((
        ...
        map(
            preceded(tag("datetime"), delimited(tag("("), trim(date), tag(")"))),
            |x| Literal::DateTime(x),
        ),
        map(
            preceded(tag("dynamic"), delimited(tag("("), trim(dynamic), tag(")"))),
            |x| Literal::Dynamic(x),
        ),
        map(
            preceded(tag("int"), delimited(tag("("), trim(integer), tag(")"))),
        ...
        map(
            preceded(tag("real"), delimited(tag("("), trim(real), tag(")"))),
            |x| Literal::Real(x),
        ),
        map(
            preceded(
            ...
            |x| Literal::Timespan(x),
        ),
        map(preceded(tag_no_case("0x"), hex_digit1), |x| {
            Literal::Long(Some(i64::from_str_radix(x, 16).unwrap()))
        }),
        map(
            terminated(decimal_number, alt((tag("days"), tag("day"), tag("d")))),
            |x| Literal::Timespan(Some(dec_to_i64(x, 1000 * 1000 * 1000 * 60 * 60 * 24))),
        ),
        map(
            terminated(decimal_number, alt((tag("hours"), tag("hour"), tag("h")))),
            |x| Literal::Timespan(Some(dec_to_i64(x, 1000 * 1000 * 1000 * 60 * 60))),
        ),
        map(
            terminated(
                decimal_number,
            ...
            |x| Literal::Timespan(Some(dec_to_i64(x, 1000 * 1000 * 1000 * 60))),
        ),
        map(
            terminated(
                decimal_number,
            ...
            |x| Literal::Timespan(Some(dec_to_i64(x, 1000 * 1000 * 1000))),
        ),
        map(
            terminated(
                decimal_number,
            ...
            |x| Literal::Timespan(Some(dec_to_i64(x, 1000 * 1000))),
        ),
        map(
            terminated(
                decimal_number,
            ...
            |x| Literal::Timespan(Some(dec_to_i64(x, 1000))),
        ),
        map(
            terminated(decimal_number, alt((tag("ticks"), tag("tick")))),
            |x| Literal::Timespan(Some(dec_to_i64(x, 100))),
        ),
        map(
            recognize(tuple((
                opt(tag("-")),
            ...
            |x: &str| Literal::Real(Some(x.parse().unwrap())),
        ),
        map(
            recognize(tuple((
                opt(tag("-")),
...
fn delim_expr(i: &str) -> IResult<&str, Expr> {
    let (i, ident) = alt((delimited(tag("("), trim(or_expr), tag(")")), ident_expr)).parse(i)?;

    fold_many0(
        alt((
            trim(preceded(
                opt(trim(tag("."))),
                delimited(tag("["), trim(expr), tag("]")),
            )),
            map(preceded(trim(tag(".")), identifier), |i| Expr::Ident(i)),
        )),
        move || ident.clone(),
        |f, i| Expr::Index(Box::new(f), Box::new(i)),
    )
    .parse(i)
}
...
fn predicate(i: &str) -> IResult<&str, Expr> {
    ...
    let (i, e) = fold_many0(
        pair(trim(alt((string_op_keyword, is_a("!=<>")))), addsub_expr),
...
fn datatable_operator(i: &str) -> IResult<&str, (Vec<(String, Type)>, Vec<Expr>)> {
    preceded(
        terminated(tag("datatable"), multispace1),
        separated_pair(
...
fn distinct_operator(i: &str) -> IResult<&str, Vec<String>> {
    preceded(
        terminated(tag("distinct"), multispace1),
        separated_list1(tag(","), trim(identifier)),
    )
    .parse(i)
}
...
fn evaluate_operator(i: &str) -> IResult<&str, (Options, String, Vec<Expr>)> {
    preceded(
        terminated(tag("evaluate"), multispace1),
        tuple((
...
fn extend_operator(i: &str) -> IResult<&str, Vec<(Option<String>, Expr)>> {
    preceded(
        terminated(tag("extend"), multispace1),
        separated_list0(
...
fn externaldata_operator(i: &str) -> IResult<&str, (Vec<(String, Type)>, Vec<String>)> {
    preceded(
        terminated(tag("externaldata"), multispace1),
        separated_pair(
            delimited(tag("("), type_mapping, tag(")")),
            multispace0,
...
fn facet_operator(i: &str) -> IResult<&str, (Vec<String>, Vec<Operator>)> {
    preceded(
        terminated(
            separated_pair(tag("facet"), multispace1, tag_no_case("by")),
            multispace1,
        ),
        pair(
            ...
            map(
                opt(preceded(
                    terminated(tag("with"), multispace0),
                    delimited(
                        tag("("),
...
fn find_operator(i: &str) -> IResult<&str, (Options, (Option<Vec<Source>>, Expr), FindProjection)> {
    preceded(
        terminated(tag("find"), multispace1),
        tuple((
            ...
            map(
                opt(preceded(
                    ...
                    alt((
                        ...
                        map(
                            preceded(
                                terminated(tag("project"), multispace1),
                                separated_list1(trim(tag(",")), identifier),
                            ),
                            |c| FindProjection::Project(c),
                        ),
                    )),
                )),
...
fn fork_operator(i: &str) -> IResult<&str, Vec<(Option<String>, Vec<Operator>)>> {
    preceded(
        terminated(tag("fork"), multispace1),
        separated_list1(
            tag(","),
            trim(alt((
                map(
...
fn join_operator(i: &str) -> IResult<&str, (Options, TabularExpression, Vec<JoinKey>)> {
    preceded(
        terminated(tag("join"), multispace1),
        tuple((
            terminated(options, multispace0),
            terminated(
                delimited(trim(tag("(")), parse_query, trim(tag(")"))),
                multispace0,
            ),
            preceded(
...
fn lookup_operator(i: &str) -> IResult<&str, (Options, TabularExpression, Vec<String>)> {
    preceded(
        terminated(tag("lookup"), multispace1),
        tuple((
            terminated(options, multispace0),
...
fn mv_apply_operator(
    i: &str,
) -> IResult<&str, (Vec<((String, String), Option<Type>)>, Vec<Operator>)> {
    preceded(
        terminated(tag("mv-apply"), multispace1),
        tuple((
            separated_list1(
                tag(","),
                trim(pair(
...
fn parse_operator(i: &str) -> IResult<&str, (Options, Expr, Vec<PatternToken>)> {
    preceded(
        terminated(tag("parse"), multispace1),
        tuple((
            terminated(options, multispace0),
...
fn parse_where_operator(i: &str) -> IResult<&str, (Options, Expr, Vec<PatternToken>)> {
    preceded(
        terminated(tag("parse-where"), multispace1),
        tuple((
            terminated(options, multispace0),
            terminated(expr, multispace0),
            preceded(terminated(tag("with"), multispace1), pattern),
...
fn parse_kv_operator(i: &str) -> IResult<&str, (Expr, Vec<(String, Type)>, Options)> {
    preceded(
        terminated(tag("parse-kv"), multispace1),
        tuple((
            terminated(expr, multispace0),
            terminated(
                preceded(
            ...
            preceded(
                terminated(tag("with"), multispace1),
                delimited(tag("("), options_with_comma_and_quoted, tag(")")),
            ),
        )),
    )
    .parse(i)
...
fn partition_operator(
    i: &str,
) -> IResult<&str, (Options, String, (Option<Source>, Vec<Operator>))> {
    preceded(
        terminated(tag("partition"), multispace1),
        tuple((
            terminated(options, multispace0),
            ...
            alt((
                map(
                    preceded(
                        ...
                        delimited(
                            tag("("),
                            separated_list0(tag("|"), trim(operator)),
                            tag(")"),
                        ),
                    ),
                    |o| (None, o),
                ...
                map(
                    separated_pair(
                        source,
                        multispace0,
                        many0(preceded(tag("|"), trim(operator))),
                    ),
                    |(s, o)| (Some(s), o),
                ),
            )),
...
fn print_operator(i: &str) -> IResult<&str, Vec<(Option<String>, Expr)>> {
    preceded(
        terminated(tag("print"), multispace0),
        separated_list0(
            tag(","),
            trim(alt((
                map(
...
fn project_away_operator(i: &str) -> IResult<&str, Vec<String>> {
    preceded(
        terminated(tag("project-away"), multispace1),
        separated_list1(tag(","), trim(identifier)),
    )
    .parse(i)
}
...
fn project_keep_operator(i: &str) -> IResult<&str, Vec<String>> {
    preceded(
        terminated(tag("project-keep"), multispace1),
        separated_list1(tag(","), trim(identifier)),
    )
    .parse(i)
}
...
fn project_rename_operator(i: &str) -> IResult<&str, Vec<(String, String)>> {
    preceded(
        terminated(tag("project-rename"), multispace1),
        separated_list1(
            tag(","),
            separated_pair(trim(identifier), tag("="), trim(identifier)),
        ),
...
fn project_reorder_operator(i: &str) -> IResult<&str, Vec<(String, Option<(bool, bool)>)>> {
    preceded(
        terminated(tag("project-reorder"), multispace1),
        separated_list1(
            tag(","),
            trim(pair(
                wildcard_identifier,
...
fn range_operator(i: &str) -> IResult<&str, (String, Expr, Expr, Expr)> {
    preceded(
        terminated(tag("range"), multispace1),
        tuple((
            terminated(identifier, multispace1),
            terminated(
                preceded(terminated(tag("from"), multispace1), expr),
                multispace1,
            ),
            terminated(
                preceded(terminated(tag("to"), multispace1), expr),
                multispace1,
            ),
...
fn reduce_operator(i: &str) -> IResult<&str, (Options, Expr, Option<Options>)> {
    preceded(
        terminated(tag("reduce"), multispace1),
        tuple((
            terminated(options, multispace0),
            terminated(
                preceded(terminated(tag("by"), multispace1), expr),
                multispace0,
            ),
            opt(preceded(
                terminated(tag("with"), multispace1),
                options_with_comma_and_quoted,
            )),
        )),
    )
    .parse(i)
...
fn render_operator(i: &str) -> IResult<&str, (String, Option<Options>)> {
    preceded(
        terminated(tag("render"), multispace1),
        tuple((
            terminated(identifier, multispace0),
            opt(preceded(
                terminated(tag("with"), multispace1),
...
fn sample_operator(i: &str) -> IResult<&str, u32> {
    preceded(terminated(tag("sample"), multispace1), u32).parse(i)
}
...
fn serialize_operator(i: &str) -> IResult<&str, Vec<(Option<String>, Expr)>> {
    preceded(
        terminated(tag("serialize"), multispace1),
        separated_list0(
            tag(","),
            trim(map(
                separated_pair(identifier, trim(tag("=")), expr),
...
fn summarize_operator(i: &str) -> IResult<&str, (Vec<(Option<String>, Expr)>, Vec<Expr>)> {
    preceded(
        terminated(tag("summarize"), multispace1),
        pair(
            separated_list0(
                tag(","),
                trim(alt((
...
fn sort_operator(i: &str) -> IResult<&str, Vec<String>> {
    preceded(
        tuple((tag("sort"), multispace1, tag("by"))),
        separated_list1(tag(","), trim(identifier)),
    )
    .parse(i)
}
    ...
    map(
        preceded(
            ...
            tuple((
                ...
                opt(terminated(
                    alt((value(true, tag("asc")), value(false, tag("desc")))),
                    multispace0,
                )),
                opt(preceded(
                    terminated(tag("nulls"), multispace1),
                    alt((value(true, tag("first")), value(false, tag("last")))),
                )),
            )),
        ),
        |(n, e, s, o)| (n, e, s.unwrap_or(false), o.unwrap_or(s.unwrap_or(false))),
    )
    .parse(i)
}
...
fn union_operator(i: &str) -> IResult<&str, (Options, Vec<Source>)> {
    preceded(
        terminated(tag("union"), multispace1),
        tuple((
            terminated(options, multispace0),
            separated_list1(
                trim(tag(",")),
...
fn operator(i: &str) -> IResult<&str, Operator> {
    alt((
        ...
        alt((
            ...
            map(parse_where_operator, |(o, e, p)| {
                Operator::ParseWhere(o, e, p)
            }),
            map(parse_kv_operator, |(e, t, o)| Operator::ParseKV(e, t, o)),
        )),
        map(partition_operator, |(o, a, (s, g))| {
            Operator::Partition(o, a, s, g)
        }),
        ...
        alt((
            ...
            map(sample_distinct_operator, |(s, c)| {
                Operator::SampleDistinct(s, c)
            }),
        )),
        alt((
...
pub fn parse_query(i: &str) -> IResult<&str, TabularExpression> {
    map(
        ...
        |(source, operators)| TabularExpression { source, operators },
    )
    .parse(i)
}
