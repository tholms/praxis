...
pub fn render(f: &mut Frame, area: Rect, app: &App) {
    ...
    if let Some(session) = state.active_session() {
        ...
        register_browse_hits(app, panes.list, panes.detail, outer[1]);
        ...
        if has_terminal {
            reg.gap(4);
            reg.chip(action::TERMINAL, MouseAction::NodesHint(NodesHintAction::Terminal));
            reg.chip(" terminal", MouseAction::NodesHint(NodesHintAction::Terminal));
            hint_spans.push(Span::raw("    "));
            hint_spans.push(Span::styled(action::TERMINAL, key_style));
        ...
        reg.gap(4);
        reg.chip(action::SESSIONS, MouseAction::NodesHint(NodesHintAction::Sessions));
        reg.chip(&sessions_label, MouseAction::NodesHint(NodesHintAction::Sessions));
        hint_spans.push(Span::raw("    "));
        hint_spans.push(Span::styled(action::SESSIONS, key_style));
...
fn register_browse_hits(app: &App, list_area: Rect, detail_area: Rect, _hints_area: Rect) {
    //
    // Pane hits first; split border last so drag wins on the divider.
    //
    app.hits_register(
        list_area,
